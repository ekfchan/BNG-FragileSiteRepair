...
my %inputs = (); 
GetOptions( \%inputs, 'x|xmap=s', 'q|qcmap=s', 'r|rcmap=s', 'e|errbin=s', 'o|output|prefix=s', 'bed|b:s', 'round:i', 'maxlab:i', 'maxfill:i', 'wobble:i', 'n:i', 'alignmolAnalysis=s', 'minRatio=s', 'maxOverlap:i', 'maxOverlapLabels:i', 'pvalue:s','MoleculesMode','MapsMode','RefAligner:s','maxiter:i','molStats:s','err=s'); 
...
if (scalar(@secondContigList)>0 && $inputs{round}<=$inputs{maxiter}) {
	...
	while ($inputs{round} < $inputs{maxiter}) {
		...
		$cmd = "$inputs{RefAligner} -i ".$outName2."_unmapped.bnx -i $outName2"."_mapped.bnx -o $outName2"."_q -merge -f -stdout -stderr -maxthreads $cpuCount 2>&1";
		print "\tRunning command: $cmd\n";	
		system($cmd);
		print "\n";
		
		#$cmd = "$inputs{RefAligner} -i ".$outName2."_unmapped.cmap -i $outName2"."_q.cmap -o $outName2"."_q -merge -f -stdout -stderr -maxthreads $cpuCount 2>&1";
		#print "\tRunning command: $cmd\n";	
		#system($cmd);
		#print "\n";
	...
	if ($inputs{round} == $inputs{maxiter}) {
		...
		#$cmd = "$inputs{RefAligner} -i ".$outName3."_unmapped.cmap -i $outName3"."_q.cmap -o $outName3"."_q -merge -f -stdout -stderr 2>&1";
		#print "\tRunning command: $cmd\n";	
		#system($cmd);
		#print "\n";		
