...
#Usage: perl scoreStitchPositionsBED_v3.pl <--bed stitchPositions.bed> <--xmap alignref_final.xmap> <--qcmap alignref_final_q.cmap> <--rcmap alignref_final_r.cmap> <--alignmol alignmol.xmap> <--output output directory> [<--n cpu cores>]
#Example: 
...
my %inputs = (); 
GetOptions( \%inputs, 'xmap=s', 'qcmap=s', 'rcmap=s', 'cmap=s', 'alignmol=s', 'bed:s', 'n:i','output:s'); 

if( !exists $inputs{xmap} | !exists $inputs{qcmap} | !exists $inputs{rcmap} | !exists $inputs{alignmol} | !exists $inputs{output} ) {
	print "Usage: perl scoreStitchPositionsBED_v3.pl <--bed stitchPositions.bed> <--xmap alignref_final.xmap> <--qcmap alignref_final_q.cmap> <--rcmap alignref_final_r.cmap> <--alignmol alignmol.xmap> <--output output directory> [<--n cpu cores>]\n"; 
	exit 0; 
}
...
foreach my $key ("xmap","qcmap","rcmap","cmap","alignmol", "bed", "output") {
...
my $molxmapFileIn = $inputs{alignmol};
my @molxmap;
...
print STATS "#BedLine,Type,RefId,RefStart-End,RefLabelStart-End,MapId,MapStart-End,MapLabelStart-End,TotalHits,UniqueMolecules,TotalConf,AvgConfPerHit,AvgConfPerMol,Score,AltScore,MoleculeMatches\n";
	...
	foreach my $xmapEntry_ref (defined($xmap[$bedCmapId])?@{$xmap[$bedCmapId]}:()) {
		...
		if ($xmapEntry{'RefContigID'} eq $bedCmapId) {
			...
			if ( ($refLabels =~ /\b$bedStartLabelId\b/) && ($refLabels =~ /\b$bedEndLabelId\b/) ) {
				...
				my $genomeMapStart = $qryLabelsArray[0];
				my $genomeMapEnd = $qryLabelsArray[-1];
