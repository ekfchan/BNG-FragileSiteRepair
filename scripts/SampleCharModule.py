...
class autoNoise(mthread.jobWrapper) :
    ...
    def isBadErrorParams(self, noise):
        #add not noise for case of empty dict, which readNoiseParameters will return if it can't read the .err file
        if not noise or noise["sr"]>0.1 or (noise["sd"]>0 and noise["sf"]>0.25) or noise["sd"]>0.1 or noise["sf"]>0.5 :
...
def readNoiseParameters(input_file):
    ...
    if len(data) > 13 : #be sure that we don't get IndexError
        return(dict([("FP",data[1]), ("FN",data[2]), ("sf",data[3]), ("sd",data[4]), ("bpp",data[5]), ("sr",data[13]), ("readparameters",input_file+".errbin")]))
    elif len(data) > 5 : #assume sr is missing in this case, but rest are correct
        return(dict([("FP",data[1]), ("FN",data[2]), ("sf",data[3]), ("sd",data[4]), ("bpp",data[5]), ("readparameters",input_file+".errbin")]))
