...
util.setVersion("$Id: Pipeline.py 4288 2015-11-19 17:54:37Z wandrews $")
...
class varsPipeline():
    ...
    def __init__(self):
        ...
        self.doScanScale = False #see def checkScanScaling
        self.rescaleSuffix = "_rescaled" #the file will be <path+prefix><this>.bnx
        self.groupSV = True #now a command line arg, -s
    ...
    def parseCommandLine(self):
        ...
        parser.add_argument('-j', help='Threads per job [default 1]', dest = 'maxthreads', default = 1,type=int)
        parser.add_argument('-N', help='Number of split bnx files; number of pairwise jobs is N*(N-1)/2 (optional, default: -T)', default = -999,type=int)
        #parser.add_argument('-G', help='Compute and use global statistics on input molecules, requires RefAligner >= 2329.', default=1, action='store_false') #I'm retiring this option since it's not used and the default should always be used
        ...
        parser.add_argument('-i', help='Number of extension and merge iterations (default=1, must be in range [0,10], use 0 to skip)', dest='iter', default=1, type=int)
        parser.add_argument('-I', help='File with listed paths for image processing; no longer supported--do not use (use without .bnx)', dest='img', default=None)
        ...
        bypassHelp = 'Skip steps, using previous result. <= 0:None, 1:ImgDetect, 2:NoiseChar/Subsample, 3:Pairwise, 4:Assembly, 5:RefineA, 6:RefineB, (7:RefineNGS -f only), 7:merge0, 8+(i-1)*2:Ext(i), 9+(i-1)*2:Mrg(i), N+1:alignmol'
...
#        parser.add_argument('-c', help='Remove intermediate contig files', dest='cleanup', action='store_true')
        ...
        parser.add_argument('-g', help='Organism genome size estimate in megabases, used for tuning assembly parameters [optional, if > 0, will modify parameters, if == 0, ignored, must be float]', dest='size', default=0, type=float)
        ...
        parser.add_argument('-L', help='Lambda phage is spiked in, used for molecule scaling (only used with -I input)', dest='lambdaRef', default=None)
        ...
        parser.add_argument('-f', help='Directory which contains NGS contigs as cmaps for use in refineNGS (no longer supported).', dest='ngsarg', default=None)
        parser.add_argument('-F', help='When using -f, disable min contigs check for all stages prior to refineNGS (no longer supported).', dest='ngsskiparg', action='store_true')
        ...
        parser.add_argument('-Y', help='Disable scan scaling in auto noise (default on with reference and -y)', default=True, action='store_false')
        ...
        parser.add_argument('-H', help='Use HG19 (human genome) as reference, loaded from Analysis/SV/CopyNumberProfiles/DATA. Overrides -r argument.', default=False, action='store_true')
        ...
        #self.GlobalStats = result.G #retired
        ...
        if self.noiseOnly and not self.autoNoise :
            ...
            self.autoNoise = True
        self.doNoise = result.noise #now _no longer_ assume doNoise if noiseOnly
        ...
        self.load_hg19 = result.H
        self.doScanScale = result.Y
        self.groupSV = result.groupsv
    ...
    def checkInputs(self):
        if self.load_hg19 : #if this parameter is used, it will override self.ref
            if self.ref : #warn user their reference is being ignored
                self.message += '  WARNING: ignoring reference (%s) because -H used--using HG19 instead\n' % self.ref
                self.warning += 1
    ...
    def checkFlow(self):
        ...
        if self.subsampleMb < 0:
            ...
            self.error += 1
        if self.nExtensionIter < 0 or self.nExtensionIter > 10 :
            self.message += '  ERROR:   -i (number of extension-merge stages) must be 0 or positive number <= 10\n'
            self.error += 1
    ...
    def assignFileTargets(self):
        ...
        #if self.GlobalStats : #-G option: bnxStatsFile defaults to None -- retired option, always use
    ...
    def prerunLog(self):
        ...
        raver = util.getRefAlignerVersion(self.RefAlignerBinOrig) #string
        if raver :
            self.updatePipeReport( "  RefAligner Version: %s\n\n" % raver )
        else : #None or False means unable to get version
            raver = ""
            err = "Unable to determine RefAligner version"
            self.updatePipeReport( err+"\n\b" )
            util.LogError("warning", err)
        #check for other svn strings, store integer
        r = raver.replace("SVNversion=","")
        if r.find(":") != -1 : #remove after :
            r = r[:r.find(":")]
        if len(r) and util.getIntFromString(r[-1]) == None : #last char is string; remove
            r = r[:-1]
        self.refaligner_version = (util.getIntFromString(r) if util.getIntFromString(r) != None else 0)
...
class DNPipeline():
	...
	util.LogStatus("progress", "stage_complete", "AsyncWait")
        ...
        if execute and varsP.runSV and (varsP.ngsInDir or varsP.nExtensionIter or not varsP.skipFinalRefine) :
            ...
            charnoise = scm.readNoiseParameters(plist[0] if len(plist) else "") #empty string will return {}
    ...
    def finalizePipeline(self, varsP):
    ...
    def runCharacterize(self, varsP):
        ...
        if varsP.executeCurrentStage:
            ...
            return cm.dummyCharacterize(varsP) #just get a path for SV noise parameters
