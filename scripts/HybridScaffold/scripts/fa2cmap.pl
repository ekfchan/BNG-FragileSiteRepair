...
BEGIN {
        ...
        if ($] >= 5.010000 && $] <= 5.011000) {
                ...
                print "ERROR: Unsupported Perl version found: $]. Supported Perl versions include 5.10.X and 5.14.X and 5.16.X\n";
                die "ERROR: Unsupported Perl version found: $]. Supported Perl versions include 5.10.X and 5.14.X and 5.16.X\n";
                exit; }
        unshift @INC, $module_path2;
        unshift @INC, $lib4;
...
$origInput = basename($opts{i});
#print "$origInput\n";

mkdir "$outDir/fa2cmap" if (! -e "$outDir/fa2cmap");
$filename = $outDir."/fa2cmap/".$origInput;
$filename =~ s/(\S+)\.\w+$/$1_$current_enzyme/;
$filename = $filename."_".$min_length."Kb_".$min_labels."labels.cmap";
$filename_key = $outDir."/fa2cmap/".$origInput;
$filename_key =~ s/(\S+)\.\w+$/$1_$current_enzyme/;
$filename_key = $filename_key."_".$min_length."Kb_".$min_labels."labels_key.txt";

open($KEY, ">$filename_key") || dieLog ("ERROR: Can't write $filename_key: $!\n");
