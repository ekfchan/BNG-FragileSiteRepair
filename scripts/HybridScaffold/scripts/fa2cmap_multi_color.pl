# $Id: fa2cmap_multi_color.pl 4279 2015-11-16 19:53:05Z apang $
#!/usr/bin/perl -w
#################################################################################
...
#   -m : Filter: Minimum labels (Integer, default: 0)                           #
...
my ($FASTA, $CMAP, $KEY, $BED, $SUMMARY, $STATUS, $ERROR);
...
my ($help, $version, $web, $keyfile, $input, $output, $nbase_bed, $bedfile);
my (@loci, @loci_tmp);
my (%color, %color_to_enzyme_sequence);
...
print $SUMMARY("Version:\t", '$Id: fa2cmap_multi_color.pl 4279 2015-11-16 19:53:05Z apang $', "\n");
...
print $SUMMARY("Minimum labels:\t$min_labels\n");
print $SUMMARY("Minimum size (kb):\t$min_length\n");
for(my $i = 0; $i < @enzymes; $i++){
...
print $SUMMARY("CMapId\tChannel\tLength(Mb)\tFreq(nicks/100kb)\tGC%\tN%\n");
#print $SUMMARY("------------------------------------------------------------------\n");
...
while(my $line = <$FASTA>){
	...
	if($line =~ /^>/){
		...
		if($count != 0){
			...
			if($nbase_bed){
				$nGaps->{$fastaHeader_previous} = getNGaps($seq, $minNGapSize);
			}
		}
...
if($seq_length){
	...
	if($nbase_bed){
		...
		$nGaps->{$fastaHeader} = getNGaps($seq, $minNGapSize);
	}
}
close($FASTA);
close($KEY);
...
print $SUMMARY("Total length of the original sequence entries\t$total_length_original\n");
print $SUMMARY("Total length of the filtered maps\t$total_length_filtered\n");
print $SUMMARY("Map N50\t", Get_N50(\@length_filtered, $total_length_filtered), "\n");
print $SUMMARY("Map N90\t", Get_N90(\@length_filtered, $total_length_filtered), "\n");
...
foreach my $channel ( sort {$a <=> $b} @color_uniq ){
	...
	print $SUMMARY("Label to label distance N50 for channel $channel\t$distance_tmp\n");
}
my $label_distance_global = sprintf("%.1f", Get_N50(\@label_distance_filtered, $total_label_distance_filtered));
print $SUMMARY("Overall label to label distance N50\t$label_distance_global\n");

foreach my $channel ( sort {$a <=> $b} @color_uniq ){
	my $distance_stddev_tmp = sprintf("%.1f", StdDev(\@{$label_distances_by_channel[$channel]}));
	print $SUMMARY("Standard deviation of label to label distance for channel $channel\t$distance_stddev_tmp\n");
}
my $distance_stddev_global = sprintf("%.1f", StdDev(\@label_distance_filtered));
print $SUMMARY("Standard deviation of overall label to label distance\t$distance_stddev_global\n");
...
foreach my $channel ( sort {$a <=> $b} @color_uniq ){
	...
	print $SUMMARY("Global nick frequency (nicks/100kb) for channel $channel\t$density_tmp\n");
}
if($total_length_filtered){
...
print $SUMMARY("Global nick frequency (nicks/100kb)\t$density_global\n");
...
sub Version{
	# $Id: fa2cmap_multi_color.pl 4279 2015-11-16 19:53:05Z apang $
	my $REV = '$Id: fa2cmap_multi_color.pl 4279 2015-11-16 19:53:05Z apang $';
	($REV) = $REV =~ /\$Id: (.*)\$/;
	if($REV){
		print $REV, "\n";
	}
...
sub printNGaps{
	...
	my @chromosomes = sort (keys %$ngap_ref);
	foreach my $chromosome (@chromosomes){
		my $c_gaps_ref= $ngap_ref->{$chromosome};
		my $num_gaps = scalar @$c_gaps_ref;
		for(my $i = 0; $i < $num_gaps; $i++){
			print NG "$chromosome\t$ngap_ref->{$chromosome}->[$i]->[0]\t$ngap_ref->{$chromosome}->[$i]->[1]\n";
		}
	}
	close(NG);
}
