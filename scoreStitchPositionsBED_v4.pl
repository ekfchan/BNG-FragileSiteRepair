...
#Usage: perl scoreStitchPositionsBED_v4.pl <--bed stitchPositions.bed> <--xmap alignref_final.xmap> <--qcmap alignref_final_q.cmap> <--rcmap alignref_final_r.cmap> <--alignmolxmap alignmol.xmap> <--alignmolrcmap> <--output output directory> [<--n cpu cores>]
#Example: 
...
my %inputs = (); 
GetOptions( \%inputs, 'xmap=s', 'qcmap=s', 'rcmap=s', 'cmap=s', 'alignmolxmap=s', 'alignmolrcmap=s','bed:s', 'n:i','output:s'); 
...
if( !exists $inputs{xmap} | !exists $inputs{qcmap} | !exists $inputs{rcmap} | !exists $inputs{alignmolxmap} | !exists $inputs{output} | !exists $inputs{alignmolrcmap} ) {
	print "Usage: perl scoreStitchPositionsBED_v4.pl <--bed stitchPositions.bed> <--xmap alignref_final.xmap> <--qcmap alignref_final_q.cmap> <--rcmap alignref_final_r.cmap> <--alignmolxmap alignmol.xmap> <--alignmolrcmap> <--output output directory> [<--n cpu cores>]\n"; 
	exit 0; 
}
...
foreach my $key ("xmap","qcmap","rcmap","cmap","alignmolxmap", "alignmolrcmap", "bed", "output") {
...
print STATS "#BedLine,Type,FsiteSize,RefId,RefStart-End,RefLabelStart-End,MapId,MapStart-End,MapLabelStart-End,AvgMapCov,TotalHits,UniqueMolecules,TotalConf,AvgConfPerHit,AvgConfPerMol,Score,AltScore,MoleculeMatches\n";
