...
if ($inputs{MapsMode}) {
	...
	if (-e $finalmap && (exists $inputs{ref})) {
		...
		$cmd = "cp $scriptspath/runCharacterizeFinal.py $inputs{scripts}/; cd $inputs{output}; python $inputs{scripts}/runCharacterizeFinal.py -t $inputs{RefAligner} -r $inputs{ref} -q $finalmap -p $inputs{scripts} -n $cpuCount -a $inputs{optArgs}";
		print "\tRunning command: $cmd\n\n";
		system($cmd);
	}
	print "\n";
