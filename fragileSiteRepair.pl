...
my $prefix;
$inputs{'force'}=0;
GetOptions( \%inputs, 'bed:s', 'fasta:s', 'xmap=s', 'qcmap=s', 'rcmap=s', 'errbin=s', 'output=s', 'maxlab:i', 'maxfill:i', 'wobble:i', 'force'); 
...
if( $hasbed==0 ) {
	...
	print "BED file $inputs{bed} already provided; skipping this step.\n\n"; 
	$bed = $inputs{bed}; 
}
